			...
			if ((sender instanceof Player)) {
				...
				if (args.length == 0) {
					...
					if((args[0].equalsIgnoreCase("forcedm")) && (GamePlayer.getPlayer(p.getName()).getRank().has(p, Rank.ADMIN, true))) {
						Bukkit.broadcastMessage(ChatUtils.modulate("SurvivalGames", "Deathmatch has been forced by " + GamePlayer.getPlayer(p.getName()).getChatPrefix() + p.getName()));
					...
					if((args[0].equalsIgnoreCase("reload")) && (GamePlayer.getPlayer(p.getName()).getRank().has(p, Rank.ADMIN, true))) {
						...
						if(args[1].equalsIgnoreCase("ch")) {
							Core.getChests().save();
						}
					} else if((args[0].equalsIgnoreCase("game")) && (GamePlayer.getPlayer(p.getName()).getRank().has(p, Rank.ADMIN, true))) {
					    if(args[1].equalsIgnoreCase("start")) {
					        Core.get().getGame().forceStart(p);
					        Bukkit.broadcastMessage(ChatUtils.modulate("SurvivalGames", GamePlayer.getPlayer(p.getName()).getChatPrefix() + p.getName() + " &ahas started the game."));
                        } else if(args[1].equalsIgnoreCase("stop")) {
					        if(Core.get().getGame().getState() == GameState.LOBBY || Core.get().getGame().getState() == GameState.ENDING || Core.get().getGame().getState() == GameState.DEAD) {
                                p.sendMessage(ChatUtils.modulate("SurvivalGames", "&cThere is no game running!"));
                            } else {
                                Core.get().getGame().end();
                                Bukkit.broadcastMessage(ChatUtils.modulate("SurvivalGames", GamePlayer.getPlayer(p.getName()).getChatPrefix() + p.getName() + " &chas stopped the game."));
                            }
                        }
                    }
				} else if((args.length == 3) && (GamePlayer.getPlayer(p.getName()).getRank().has(p, Rank.ADMIN, true))) {
				    if(args[0].equalsIgnoreCase("map")) {
				        ...
				        if(args[1].equalsIgnoreCase("setmap")) {
				            if(!Core.getMaps().contains(args[2])) {
                                p.sendMessage(ChatUtils.modulate("SurvivalGames", "&e" + args[2] + " &cis not a valid map."));
                            } else {
				                Core.get().getGame().setCurrentMap(getMapByName(args[2]));
				                p.sendMessage(ChatUtils.modulate("SurvivalGames", "&aSuccessfully set the map to &e" + args[2]));
				                Bukkit.broadcastMessage(ChatUtils.modulate("SurvivalGames", GamePlayer.getPlayer(p.getName()).getChatPrefix() + p.getName() + " &ahas set the map to &e" + args[2]));
                            }
                        }
                    }
				} else {
